...
public class ListenerImplemention implements ITestListener{
	...
	public void onTestFailure(ITestResult result) {
		...
		try {
			String path=w.screenshot(BaseClass.listenerdri, folname);
	...
	public void onStart(ITestContext context) {
		...
		ExtentSparkReporter rep=new ExtentSparkReporter("./ExtentReport\\Report-"+new JavaUtility().modifiedDate()+".html");
		rep.config().setDocumentTitle("Execution Report");
		rep.config().setTheme(Theme.DARK);
		rep.config().setReportName("V Tiger Report");
		...
		report.setSystemInfo("reporter name","Lisa");
	}
